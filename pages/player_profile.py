...
position_mapping = {
    ...
    "Left Back": "Number 3", "Right Back": "Number 3", "Left Wing Back": "Number 3", "Right Wing Back": "Number 3",
    "Defensive Midfielder": "Number 8", "Left Defensive Midfielder": "Number 8", "Right Defensive Midfielder": "Number 8",
    ...
    "Left Attacking Midfielder": "Number 10", "Winger": "Winger", "Right Midfielder": "Winger", "Left Midfielder": "Winger",
    "Left Wing": "Winger", "Right Wing": "Winger", "Centre Forward": "Runner", "Left Centre Forward": "Runner",
    "Right Centre Forward": "Runner", "Goalkeeper": "Goalkeeper"
}
...
ALL_METRICS = [
    ...
    'player_season_obv_gk_90', 'player_season_forward_pass_ratio', 'player_season_forward_pass_proportion',
    'player_season_scoring_contribution_90', 'player_season_fouls_won_90', 'player_season_pressures_90',
    'player_season_counterpressures_90', 'player_season_aggressive_actions_90'
]
...
def get_similar_players_across_leagues(df, target_player_name, selected_metrics, position):
    df_pos_all = df[df['mapped_position'] == position].copy()
    df_pos_all = df_pos_all.dropna(subset=selected_metrics)

    if target_player_name not in df_pos_all['player_name'].values:
        return pd.DataFrame()

    scaler = StandardScaler()
    metric_values = scaler.fit_transform(df_pos_all[selected_metrics])
    target_index = df_pos_all[df_pos_all['player_name'] == target_player_name].index[0]
    similarities = cosine_similarity([metric_values[target_index]], metric_values)[0]
    df_pos_all['similarity'] = similarities

    similar_players = df_pos_all[df_pos_all['player_name'] != target_player_name] \
        .sort_values(by='similarity', ascending=False).head(5)

    return similar_players[['player_name', 'League', 'Season', 'similarity', 'player_season_minutes'] + selected_metrics]
        ...
        if len(selected_metrics) < 10:
            ...
            if values:
                ...
                fig.update_layout(
                    ...
                    yaxis=dict(tickfont=dict(color='white'), categoryorder='array', categoryarray=labels[::-1]),
                    showlegend=False
                )
            ...
            with col1:
                st.markdown(f"""
                <div class="metric-card"><div class="metric-title">Overall Rank</div>
                <div class="metric-value">{overall_rank:.0f}%</div></div>
                """, unsafe_allow_html=True)
            with col2:
                st.markdown(f"""
                <div class="metric-card"><div class="metric-title">Position</div>
                <div class="metric-value">{player_position}</div></div>
                """, unsafe_allow_html=True)
            with col3:
                st.markdown(f"""
                <div class="metric-card"><div class="metric-title">Elite Metrics</div>
                <div class="metric-value">{metrics_above_70}</div></div>
                """, unsafe_allow_html=True)
            with col4:
                st.markdown(f"""
                <div class="metric-card"><div class="metric-title">Minutes</div>
                <div class="metric-value">{minutes_played:,}</div></div>
                """, unsafe_allow_html=True)

            # Similar players
            st.markdown("### 🔍 Similar Players (Across All Leagues and Seasons)")
            similar_df = get_similar_players_across_leagues(df, selected_player, selected_metrics, player_position)
            if not similar_df.empty:
                st.dataframe(similar_df.style.format({'similarity': '{:.2f}'}), use_container_width=True)
            else:
                st.info("No similar players found (ensure the player has full metric data).")
