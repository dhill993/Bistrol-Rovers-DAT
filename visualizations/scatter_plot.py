...
def create_scatter_chart(df, league, season, player, player_position, x_metric, y_metric, min_age, max_age, min_minutes, max_minutes):
    ...
    if player not in df['Player Name'].values:
    ...
    non_top_players = df[~df['Player Name'].isin(top_players['Player Name'])]
    ...
    player_row = df[df['Player Name'] == player]
    if not player_row.empty:
        if player in top_players['Player Name'].values:
            # Remove the player from top_players to avoid duplicate styling
            top_players = top_players[top_players['Player Name'] != player]
